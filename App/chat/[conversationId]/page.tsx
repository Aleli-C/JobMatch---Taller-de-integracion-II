...
export default async function ChatPage({ params }: PageProps) {
  ...
  const chatsDb = await getChatsByUserId(session.id);
  ...
  const otherDb = activeDb.usuario1Id === session.id ? activeDb.usuario2 : activeDb.usuario1;
  ...
  async function sendMessageAction(formData: FormData) {
    "use server";
    if (!session) return; // Agrega una verificación al inicio de la función para asegurar que session no es nulo.
    ...
    // Refrescamos para mostrar el nuevo mensaje
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <form ref={formRef} action={sendMessageAction} className="flex-1 flex flex-col">
          ...
          <ChatMain
            ...
            onToggleInfoPanel={() => setIsInfoPanelVisible((v) => !v)}
          />
        </form>
      </div>
