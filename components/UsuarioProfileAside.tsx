...
// Importamos el hook que creamos en page.tsx
import { useUser } from '../page'; // AJUSTA LA RUTA SI ES NECESARIO

const ProfileAside = () => {
  // Obtenemos los datos del usuario del contexto
  const { user } = useUser();
  
  // Si user es null (aunque ya manejamos la carga y error en el padre, TS lo requiere)
  if (!user) return null; 
