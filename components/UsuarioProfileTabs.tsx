...
import ProfileHistory, { PublicationView, TopicView } from "./ProfileHistory";
...
type TabsProps = { historyData?: HistoryData };
 ...
 */
export default function ({ historyData }: TabsProps) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'historial':
        return (
          <div className="p-4">
            {/* Recibe por props (lo agregamos en el paso 2 de este commit) */}
            {historyData ? (
              <ProfileHistory
                forumHistory={historyData.forumHistory}
                publications={historyData.publications}
              />
            ) : (
